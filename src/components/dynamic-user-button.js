...
class DynamicUserButton {
    ...
    setVisitorState(button) {
        ...
        const text = button.querySelector('.login-btn-text, .user-name');
        ...
        button.className = 'header-login-btn ' + (button.classList.contains('mobile') ? 'mobile' : 'desktop');
    ...
    updateMenuContent() {
        ...
        if (this.isAdmin) {
            ...
            menuItems = [
                { icon: 'fas fa-tachometer-alt', label: 'Painel Administrativo', action: 'dashboard', class: 'admin-item' },
                { icon: 'fas fa-user-cog', label: 'Gerenciar conta', action: 'account', class: 'account-item' },
                { icon: 'fas fa-sign-out-alt', label: 'Sair', action: 'logout', class: 'logout-item' }
            ];
        } else {
...
// Initialize globally
window.DynamicUserButton = DynamicUserButton;
// Temporarily disabled to prevent conflicts with app.js logic
// window.dynamicUserButton = new DynamicUserButton();
console.log('DynamicUserButton class disponibilizada globalmente (instancia desabilitada temporariamente)');